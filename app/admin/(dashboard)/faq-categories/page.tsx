...
export default function FaqCategoriesPage() {
  ...
  const fetchCategories = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/faq-categories").then((r) => r.json());
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    await fetch(`/api/admin/faq-categories/${deleteId}`, { method: "DELETE" });
    setDeleteId(null); fetchCategories();
  };

  return (
    <div>
      <PageHeader title="FAQ Categories" subtitle="Manage categories for frequently asked questions">
      ...
      <AdminCard className="rounded-2xl! mb-6">
        <div className="p-4 flex items-center gap-3">
          <HelpCircle className="h-4 w-4 shrink-0 text-[hsl(var(--adm-primary))]" />
          <input value={newName} onChange={(e) => setNewName(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && handleAdd()}
            placeholder="New category name… (e.g. Reservations, Amenities, Dining)"
      ...
      {loading ? (
        <div className="space-y-2">{[...Array(4)].map((_, i) => <div key={i} className="h-14 rounded-2xl adm-skeleton" />)}</div>
      ) : categories.length === 0 ? (
        <AdminCard>
          <div className="flex flex-col items-center py-16 gap-3">
            <HelpCircle className="h-12 w-12 text-[hsl(var(--adm-muted-foreground)/0.3)]" />
        ...
        <AdminCard className="rounded-2xl! overflow-hidden">
          <div className="divide-y divide-[hsl(var(--adm-border)/0.4)]">
            {categories.map((cat, i) => (
              <motion.div key={cat._id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="rounded-lg p-1.5 bg-[hsl(var(--adm-primary)/0.1)]">
                  <HelpCircle className="h-3.5 w-3.5 text-[hsl(var(--adm-primary))]" />
                </div>
                {editId === cat._id ? (
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {editId === cat._id ? (
                    <>
                      <button onClick={() => handleEdit(cat._id)} disabled={saving} className="rounded-lg p-1.5 text-[hsl(var(--adm-success))] hover:bg-[hsl(var(--adm-success)/0.1)] transition-colors"><Check className="h-3.5 w-3.5" /></button>
                      <button onClick={() => setEditId(null)} className="rounded-lg p-1.5 text-[hsl(var(--adm-muted-foreground))] hover:bg-[hsl(var(--adm-accent))] transition-colors"><X className="h-3.5 w-3.5" /></button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => { setEditId(cat._id); setEditName(cat.name); }} className="rounded-lg p-1.5 text-[hsl(var(--adm-muted-foreground))] hover:bg-[hsl(var(--adm-accent))] hover:text-[hsl(var(--adm-primary))] transition-colors"><Pencil className="h-3.5 w-3.5" /></button>
                      <button onClick={() => setDeleteId(cat._id)} className="rounded-lg p-1.5 text-[hsl(var(--adm-muted-foreground))] hover:bg-[hsl(var(--adm-destructive)/0.1)] hover:text-[hsl(var(--adm-destructive))] transition-colors"><Trash2 className="h-3.5 w-3.5" /></button>
                    </>
                  )}
                </div>
              </motion.div>
      ...
      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete}
        title="Delete FAQ Category" message="Delete this category? Existing FAQs using it will keep their category text."
