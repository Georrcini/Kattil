...
export async function GET() {
  try {
    await connectDB();
    const categories = await FaqCategory.find({ active: true })
      .sort({ order: 1, name: 1 })
      .lean();
    return apiSuccess({ categories });
  } catch (error) {
    console.error("[GET /api/admin/faq-categories]", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await FaqCategory.findOne({ slug });
    if (existing) return apiError("A category with this name already exists", 409);

    const count    = await FaqCategory.countDocuments();
    const category = await FaqCategory.create({ name: name.trim(), slug, order: order ?? count });
    return apiSuccess(category, 201);
  } catch (error) {
    console.error("[POST /api/admin/faq-categories]", error);
    return handleApiError(error);
  }
}
